...
## Task 8 Race/Retry Review Fixes
- Adopted resolved chat conversation ids synchronously inside `ChatView` before awaiting query invalidation, promoted that local id into transport/hydration state, and made `submit()` read the locally resolved id ref so quick resend paths stay on the persisted thread.
- Added regression coverage proving a delayed `invalidateQueries()` does not cause the next submit to fall back to a null/new conversation id.
- Marked upload validation failures as non-retryable, hid Retry for those local validation errors, and re-ran validation inside `retryUpload()` so invalid files stay rejected locally without posting to `/api/upload`.
- Preserved transient upload retry behavior by keeping network/server upload failures retryable and adding hook coverage for a failed upload succeeding on retry.

### Verification
- `pnpm vitest run tests/components/chat-view.test.tsx tests/components/upload-dropzone.test.tsx tests/hooks/use-conversation-messages.test.tsx`
- `pnpm vitest run tests/components/chat-view.test.tsx tests/components/upload-dropzone.test.tsx tests/hooks/use-conversation-messages.test.tsx tests/hooks/use-upload-queue.test.tsx`
- `pnpm typecheck`
- `pnpm exec eslint --max-warnings=0 components/chat/chat-view.tsx components/upload/upload-dropzone.tsx hooks/use-upload-queue.ts tests/components/chat-view.test.tsx tests/components/upload-dropzone.test.tsx tests/hooks/use-upload-queue.test.tsx`

## Task 8 Chat Threading Race Fixes
- Guarded chat conversation adoption with a request-scoped pending request record in `ChatView`, so streaming completion only re-adopts a resolved `x-conversation-id` when the active thread still matches the request (while still allowing the request-owned null→new conversation case).
- Finalized pending request cache invalidation for non-OK startup responses too, so first-message startup failures with `x-conversation-id` still attach the persisted thread client-side and retries stay on that conversation.
- Extended `AppError`/`jsonError()` to preserve safe response headers, and updated `ChatService.streamChat()` to throw startup failures with `x-conversation-id` after a conversation/message were already persisted.
- Added regression coverage for stale stream completion after conversation switching, startup-error conversation adoption in `ChatView`, structured route error headers, and service-level startup failure propagation.

### Verification
- `pnpm vitest run tests/components/chat-view.test.tsx tests/api/chat-route.test.ts tests/unit/chat-service.test.ts`
- `pnpm vitest run tests/components/chat-view.test.tsx tests/components/sidebar.test.tsx tests/components/upload-dropzone.test.tsx tests/components/document-library.test.tsx tests/components/settings-panel.test.tsx tests/hooks/use-conversation-messages.test.tsx tests/hooks/use-documents.test.tsx tests/api/messages-route.test.ts tests/unit/chat-service.test.ts`
- `pnpm typecheck`
- `pnpm exec eslint --max-warnings=0 app/api/chat/route.ts components/chat/chat-view.tsx lib/http/api-errors.ts lib/http/api-response.ts lib/services/chat-service.ts tests/api/chat-route.test.ts tests/components/chat-view.test.tsx tests/unit/chat-service.test.ts`

## Task 8 Reliability Review Fixes
- Moved `useDocuments().reindexDocument` cache refresh from success-only handling to `onSettled`, so documents/workflows are invalidated after both successful reindex requests and server error responses that may still leave workflow state behind.
- Added a hook regression test covering a failed reindex response that still requires cache invalidation to prevent stale workflow state and duplicate reindex attempts.
- Hardened upload XHR `onload` parsing in `useUploadQueue()` with a JSON parse fallback guard so non-JSON server error bodies reject cleanly as `Upload failed.` and transition queue items into the retryable error state.
- Added a hook regression test covering non-JSON upload failures to prove uploads no longer stay stuck in `uploading`.

### Verification
- `pnpm vitest run tests/hooks/use-documents.test.tsx tests/components/document-library.test.tsx tests/hooks/use-upload-queue.test.tsx tests/components/upload-dropzone.test.tsx`
- `pnpm typecheck`
- `pnpm exec eslint hooks/use-documents.ts hooks/use-upload-queue.ts tests/hooks/use-documents.test.tsx tests/hooks/use-upload-queue.test.tsx --max-warnings=0`

## Task 8 Citation Security and Loading/Error State Fixes
- Sanitized citation/source links in `components/chat/message-list.tsx` via `lib/chat/citation-url.ts`, allowing only `http:`, `https:`, and safe relative URLs while rendering unsafe `javascript:`/`data:` values as plain text without an `href`.
- Threaded conversation/document/upload-history loading and error states from hooks through `AppShell` into `Sidebar`/`ConversationList` and `DocumentLibrary`, replacing misleading empty states with accessible `role="status"` and `role="alert"` panels.
- Exposed upload history query errors from `useUploadQueue()` so the upload-history pane can distinguish loading/failure from a true empty history.
- Added regression coverage for unsafe citation URLs, safe relative citation links, sidebar loading/error states, document-library loading/error states, upload-history loading/error states, and upload-history hook failures.

### Verification
- `pnpm vitest run tests/components/chat-view.test.tsx tests/components/sidebar.test.tsx tests/components/document-library.test.tsx tests/components/upload-dropzone.test.tsx tests/hooks/use-upload-queue.test.tsx`
- `pnpm typecheck`
- `pnpm eslint components/chat/message-list.tsx components/common/app-shell.tsx components/documents/document-library.tsx components/sidebar/conversation-list.tsx components/sidebar/sidebar.tsx hooks/use-upload-queue.ts lib/chat/citation-url.ts tests/components/chat-view.test.tsx tests/components/sidebar.test.tsx tests/components/document-library.test.tsx tests/hooks/use-upload-queue.test.tsx`

## Task 8 Reindex Duplicate and Workflow Pagination Fixes
- Replaced the single pending reindex id flow with per-document pending tracking in `useDocuments`/`AppShell`, and updated `DocumentLibrary` to disable/spin only the in-flight rows while duplicate submissions for the same document are ignored until settlement.
- Scoped workflow reads in `useDocuments` to the currently loaded document ids only, added bounded `pageSize` + `documentIds` query support to `/api/workflows`, and taught `WorkflowService.listPublicWorkflows()` to return only the latest workflow per requested document while preserving the sanitized public DTO.
- Added regression coverage for duplicate reindex suppression, multi-row pending UI, document-scoped workflow fetching, workflow route validation/scoping, and latest-workflow filtering.

### Verification
- `pnpm vitest run tests/components/document-library.test.tsx tests/hooks/use-documents.test.tsx tests/api/documents-route.test.ts`
- `pnpm vitest run tests/unit/document-services.test.ts -t "limits public workflows to the latest requested document executions"`
- `pnpm typecheck`
- `pnpm exec eslint --max-warnings=0 app/api/workflows/route.ts components/common/app-shell.tsx components/documents/document-library.tsx hooks/use-documents.ts lib/services/workflow-service.ts tests/api/documents-route.test.ts tests/components/document-library.test.tsx tests/hooks/use-documents.test.tsx tests/unit/document-services.test.ts`

## Task 8 Workflow Scoping Pagination Bug Fix
- Batched `useDocuments()` workflow lookups into client-side chunks of at most 100 document ids so loaded document workflow badges continue working after loading more than 100 documents.
- Added per-document workflow lookup state tracking and updated `DocumentLibrary` to fail safe: when workflow lookup is still loading or errors, re-index stays disabled and the UI shows workflow availability clearly.
- Threaded workflow lookup state through `AppShell` and added regression coverage for >100 loaded documents, chunked workflow requests, and unavailable-workflow re-index gating.

### Verification
- `pnpm vitest run tests/hooks/use-documents.test.tsx tests/components/document-library.test.tsx tests/api/documents-route.test.ts`
- `pnpm vitest run tests/hooks/use-documents.test.tsx tests/components/document-library.test.tsx tests/api/documents-route.test.ts tests/components/chat-view.test.tsx tests/components/sidebar.test.tsx tests/components/upload-dropzone.test.tsx tests/components/settings-panel.test.tsx`
- `pnpm typecheck`
- `pnpm exec eslint hooks/use-documents.ts components/documents/document-library.tsx components/common/app-shell.tsx tests/hooks/use-documents.test.tsx tests/components/document-library.test.tsx`

## Task 8 Final Citation, Markdown, and Health Findings Fixes
- Hardened `getSafeCitationUrl()` to reject protocol-relative URLs like `//attacker.example` while still allowing `http:`, `https:`, and previously supported same-origin/safe relative paths.
- Restored markdown math rendering by enabling KaTeX output, preserving inline math tokens, and moving Mermaid/custom math block previews out of invalid `pre > div` nesting into safe container blocks.
- Added explicit non-404 `/api/health` failure handling in `AppShell`/`Sidebar`, showing a danger-toned `Health unavailable` state with error copy while keeping 404 responses labeled as `Pending Task 9`.
- Added focused regression coverage for citation URL sanitization, markdown math/mermaid rendering, AppShell health error/pending states, and Sidebar health badge tones.

### Verification
- `pnpm vitest run tests/components/chat-view.test.tsx tests/components/sidebar.test.tsx tests/components/document-library.test.tsx tests/hooks/use-documents.test.tsx tests/unit/citation-url.test.ts tests/components/markdown-message.test.tsx tests/components/app-shell.test.tsx`
- `pnpm typecheck`
- `pnpm exec eslint app/layout.tsx components/chat/markdown-message.tsx components/common/app-shell.tsx components/sidebar/sidebar.tsx lib/chat/citation-url.ts tests/unit/citation-url.test.ts tests/components/markdown-message.test.tsx tests/components/app-shell.test.tsx tests/components/sidebar.test.tsx`
