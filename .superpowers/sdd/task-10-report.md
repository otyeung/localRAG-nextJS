...
## Task 10 Reindex Race and n8n Editor Bootstrap Fixes (2026-07-06)
- Added a same-document server-side reindex guard in `DocumentService.requestReindex()` that locks the document row inside the transaction, checks for existing `QUEUED`/`RUNNING`/`WAITING` ingestion workflows, and returns a structured `CONFLICT` with the active workflow id/status before a second reindex can start.
- Kept accepted-workflow reconciliation behavior intact by only creating the local workflow row after n8n accepts the run; start failures still leave no local workflow row, while post-start persistence failures still persist a reconciliation-required marker.
- Added regression coverage for the conflict path in both `tests/unit/document-services.test.ts` and `tests/api/documents-route.test.ts`.
- Changed the Compose n8n editor default to `http://localhost:5678` for host bootstrap, while preserving `http://n8n:5678` for internal Next.js webhook/REST traffic.
- Added an opt-in `docker-compose.n8n-editor.yml` override that exposes the editor only on `127.0.0.1:5678`, updated `.env.example`, and refreshed `docker/n8n/README.md` bootstrap instructions.
- Extended `tests/unit/docker-compose.test.ts` to lock the localhost editor default and localhost-only override behavior.

### Verification
- `pnpm vitest run tests/unit/docker-compose.test.ts tests/unit/document-services.test.ts tests/api/documents-route.test.ts` ✅ (70/70 passing)
- `node -e "for (const file of ['docker/n8n/workflows/ingestion.json','docker/n8n/workflows/retrieval.json']) JSON.parse(require('node:fs').readFileSync(file,'utf8')); console.log('workflow json parse ok')"` ✅
- `docker compose config --quiet` ✅
- `docker compose -f docker-compose.yml -f docker-compose.n8n-editor.yml config --quiet` ✅
- `pnpm typecheck` ✅
- `git diff --check` ✅ before commit

### Commit
- `96685e2 fix: guard reindex overlap and n8n editor bootstrap`

## Task 10 Reindex Overlap Race Fix (2026-07-06)
- Reworked `DocumentService.requestReindex()` to create a durable `QUEUED` ingestion workflow row inside the document lock transaction before calling n8n, so concurrent same-document reindex requests now see an active workflow immediately.
- Updated accepted-start handling to mutate that same workflow row with external execution metadata and keep it active as `QUEUED`/`RUNNING`; if the post-start transaction fails, a follow-up reconciliation update preserves the row with `reconciliationRequired` and `localPersistenceError` metadata instead of recreating or losing it.
- Updated reindex start-failure handling to mark the pre-created row `ERROR` so failed upstream starts do not leave unrecoverable active blockers, while preserving the existing audit/error behavior and READY-on-failed-reindex semantics.
- Added a regression test covering accepted upstream start + post-start persistence failure + immediate second reindex conflict/no second n8n call, and refreshed surrounding reindex tests for the durable-row flow.

### Verification
- `pnpm vitest run tests/unit/document-services.test.ts tests/api/documents-route.test.ts tests/unit/docker-compose.test.ts` ✅ (71/71 passing)
- `pnpm typecheck` ✅
- `git diff --check` ✅ before commit

## Task 10 Webhook Start Retry and Reconciliation Fixes (2026-07-06)
- Disabled automatic retries for n8n workflow webhook starts by adding a per-request retry override in `N8nClient` and forcing `N8nWorkflowService.startWorkflow()` to call `/webhook/*` with `retry: false`.
- Added explicit ambiguous-start detection for n8n transport/timeout failures so pre-created reindex workflow rows stay active as `QUEUED` with `reconciliationRequired: true` metadata instead of being downgraded to `ERROR`.
- Applied the same ambiguity handling to upload ingestion starts: ambiguous webhook start failures now persist upload/document/workflow state in reconciliation-required form rather than inviting duplicate retries.
- Added regression coverage in `tests/unit/n8n-client.test.ts` and `tests/unit/document-services.test.ts` for no-retry webhook starts, ambiguous reindex conflicts, and ambiguous upload reconciliation state.

### Verification
- `pnpm vitest run tests/unit/n8n-client.test.ts tests/unit/document-services.test.ts tests/api/documents-route.test.ts tests/unit/docker-compose.test.ts` ✅ (85/85 passing)
- `pnpm typecheck` ✅

## Task 10 Deleted-Document Reindex Race Fix (2026-07-06)
- Rechecked the document inside the locked reindex transaction before creating the queued workflow row, so a soft-deleted document can no longer queue reindex work or call n8n after the pre-read.
- Switched downstream reindex persistence/start logic to use the locked document snapshot for upload/file metadata and previous status.
- Added a regression test in `tests/unit/document-services.test.ts` covering a document deleted between the initial lookup and the locked transaction.

### Verification
- `pnpm vitest run tests/unit/document-services.test.ts tests/api/documents-route.test.ts tests/unit/docker-compose.test.ts` ✅ (74/74 passing)
- `pnpm typecheck` ✅
- `git diff --check` ✅ before commit
