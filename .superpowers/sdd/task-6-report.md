...
## Task 6 Audit / Reindex Final Fixes
- Removed best-effort audit swallowing from upload and reindex mutation/reconciliation paths so audit writes are required for the same local state transition; audit insert failures now abort those transitions instead of silently preserving mutated state without an audit trail.
- Kept upload start-failure temp-file cleanup intact even when the transactional audit write fails.
- Tagged reindex workflows with metadata describing the operation and prior document status, then updated `DocumentService.requestReindex()` to avoid downgrading the existing document during accepted reindex starts and to promote the document to `READY` only after successful replacement ingestion.
- Updated `WorkflowService` reconciliation so reindex workflows no longer overwrite the original upload state and only set document status on successful reindex completion.
- Expanded regression coverage for required audit writes, reindex metadata persistence, and failure/success reconciliation behavior.

## Task 6 Audit / Reindex Verification
- `pnpm vitest run tests/unit/upload-validation.test.ts tests/unit/document-services.test.ts tests/api/upload-route.test.ts tests/api/documents-route.test.ts tests/integration/seed-corpus.test.ts`
- `pnpm typecheck`

## Task 6 Audit / Reindex Result
- 5 test files passed / 41 tests passed.
- `tsc --noEmit` passed.

## Task 6 Workflow / Qdrant Remaining Fixes
- Made `WorkflowService` reconciliation resilient to temporary n8n polling failures: list/status now return the last persisted workflow state, add a sanitized degraded reconciliation marker in metadata, and avoid surfacing upstream poll outages as route-breaking errors.
- Cleared transient poll-failure markers after successful later reconciliation while keeping public workflow DTOs sanitized to the existing `reconciliationRequired` signal.
- Updated `AppQdrantClient.ensureCollection()` to validate an existing collection's vector size and distance against configured app settings and throw a clear `UPSTREAM_ERROR` on mismatch instead of silently accepting incompatible shape.
- Added regression coverage for workflow poll-failure fallback behavior and Qdrant matching/mismatched collection shape handling.

## Task 6 Workflow / Qdrant Verification
- `pnpm vitest run tests/unit/upload-validation.test.ts tests/unit/document-services.test.ts tests/unit/qdrant-client.test.ts tests/api/upload-route.test.ts tests/api/documents-route.test.ts tests/integration/seed-corpus.test.ts`
- `pnpm typecheck`

## Task 6 Workflow / Qdrant Result
- 6 test files passed / 47 tests passed.
- `tsc --noEmit` passed.

## Task 6 Workflow Reconciliation Final Fixes
- Prevented `WorkflowService` reconciliation from resurrecting soft-deleted documents by excluding `DocumentStatus.DELETED` from all reconciliation status-sync updates while still reconciling workflow and upload state.
- Moved workflow reconciliation/status-sync persistence behind transactional helpers so GET-driven workflow mutations can write audit logs in the same transaction as workflow/upload/document updates.
- Added audit coverage for reconciled status-sync writes and persisted reconciliation-issue markers from poll failures, using sanitized workflow/upload/document status metadata only.
- Added regression coverage for deleted-document preservation plus reconciliation audit logging.

## Task 6 Workflow Reconciliation Verification
- `pnpm vitest run tests/unit/upload-validation.test.ts tests/unit/document-services.test.ts tests/unit/qdrant-client.test.ts tests/api/upload-route.test.ts tests/api/documents-route.test.ts tests/integration/seed-corpus.test.ts`
- `pnpm typecheck`

## Task 6 Workflow Reconciliation Result
- 6 test files passed / 48 tests passed.
- `tsc --noEmit` passed.

## Task 6 Remaining Review Fixes
- Added a shared pre-provision rate-limit helper for Task 6 public routes and applied it before `getCurrentUser()` on upload/uploads/documents/workflows endpoints so cookie-less requests are throttled before anonymous-user creation.
- Refactored upload and reindex orchestration to start n8n before persisting new local rows where possible, persist accepted local state in final/recoverable status, and mark accepted fallback workflows with `reconciliationRequired` metadata when follow-up local writes fail.
- Kept accepted upload temp files on disk for reconciliation paths and continued removing temp files only for true pre-acceptance failures.
- Updated workflow reconciliation to detect unchanged RUNNING polls and skip no-op workflow/audit/resource writes.
- Added regression coverage for start-failure audit failures, accepted-workflow reconciliation fallbacks, new pre-provision route guards, and no-op RUNNING polls.

## Task 6 Remaining Review Verification
- `pnpm vitest run tests/unit/upload-validation.test.ts tests/unit/document-services.test.ts tests/unit/qdrant-client.test.ts tests/api/upload-route.test.ts tests/api/documents-route.test.ts tests/integration/seed-corpus.test.ts`
- `pnpm typecheck`

## Task 6 Remaining Review Result
- 6 test files passed / 58 tests passed.
- `tsc --noEmit` passed.
