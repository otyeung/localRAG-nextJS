...
import { N8N_RETRIEVAL_WEBHOOK_PATH } from '@/lib/n8n/workflow';
...
export class N8nRetrievalService {
  constructor(
    private readonly client: Pick<N8nClient, 'post'> = createN8nClient(),
  ) {}
  ...
  async retrieve(input: RetrievalInput): Promise<RetrievedChunk[]> {
    ...
    return this.client.post(`/webhook/${N8N_RETRIEVAL_WEBHOOK_PATH}`, {
      body: payload,
